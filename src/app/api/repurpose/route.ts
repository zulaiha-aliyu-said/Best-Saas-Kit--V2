        ...
        for (const [platform, optResult] of Object.entries(optimizationResults)) {
          if (optResult && typeof optResult === 'object') {
            await insertOptimizationAnalytics({
              ...
              optimized_content_length: countCharacters((optResult as any).content || ''),
              character_count: (optResult as any).metrics?.characterCount || 0,
              word_count: (optResult as any).metrics?.wordCount || 0,
              thread_created: (optResult as any).isThread || false,
              thread_count: (optResult as any).threadPosts?.length || 0,
              hashtag_count: (optResult as any).metrics?.hashtagCount || 0,
              emoji_count: (optResult as any).metrics?.emojiCount || 0,
              line_breaks_added: (optResult as any).metrics?.lineBreaksAdded || 0,
              optimizations_applied: (optResult as any).optimizations || [],
              rules_applied: {
                platform: platform,
                maxChars: (optResult as any).preview?.platform ? 280 : 0,
              },
              warnings: (optResult as any).warnings || [],
